...
public class ItemListResource {
	...
	@Path("/id/{listId}")
	@Consumes(MediaType.APPLICATION_JSON)
	public void updateItemList(@PathParam("listId") String listId, ItemList updatedList) throws UnknownHostException {
		updatedList.setLastUpdatedDate(new Date());
		Database.getInstance().getItemListCollection()
				.update(new BasicDBObject("_id", new ObjectId(listId)), new BasicDBObject(updatedList.toMapWithoutId()));
